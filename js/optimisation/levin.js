...
levin_init = function(svg) {
  _.times(Math.round(Math.random() * 100 + 1), function(){
    ...
    window.gradient.push(function(x, y, draw){
      ...
      if (draw) {
        $('#levin_playfield').svg('get').rect(l*w,
                                              t*h,
                                              (r-l)*w,
                                              (b-t)*h,
                                              {'fill': 'white',
  ...
  $('#levin_playfield').svg('get').rect(0,
                                        0,
                                        $('#levin_playfield').width(),
                                        $('#levin_playfield').height(),
                                        {'fill': 'black'});
  _.each(window.gradient, function(a){a(0, 0, true);});
}

window.fitn = _.memoize(function(x, y) {
    return _.reduce(_.map(window.gradient, function(a) { return a(x,y); }),
    ...
    (function() {
      ...
      worker.onmessage = function(event) {
        ...
        _.each(dots, function(d) {
          d.setAttribute('fill-opacity',
                         parseFloat(d.getAttribute('fill-opacity')) * 0.9);
        });
        ...
        if (new_fit > fit) {
          ...
          $('#levin_winner').text('Program: '    +data.p.toString(2)+
                                  ' Machine: '   +data.m+
                                  ' Complexity: '+data.phase);
          if (blob) {
            $('#levin_playfield').svg('get').remove(blob);
          }
          blob = $('#levin_playfield').svg('get').circle(x,
                                                         y,
                                                         5,
                                                         {fill: 'lime'});
        }
        else {
          dots.push($('#levin_playfield').svg('get').circle(
            x, y, 2, {fill: 'red', 'fill-opacity': 0.9}));
        }
        worker.postMessage('');
      };
