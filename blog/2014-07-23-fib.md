...
```{.unwrap pipe="cat | tee hide | root/static/null"}
#!/bin/sh
INPUT=$(cat)
echo -e "\n$INPUT" >> code.php
echo "" | ./root/static/null  # .unwrap to appease tests
```

`chmod +x hide`{.unwrap pipe="sh | root/static/null"}
...
```{.unwrap pipe="cat | tee init.php | root/static/null"}
<?php
require_once('code.php');

```

```{.unwrap pipe="tee runphp | root/static/null"}
#!/bin/sh
cat init.php /dev/stdin | php
```

`chmod +x runphp`{.unwrap pipe="sh | root/static/null"}
...
```{.unwrap pipe="tee tag | root/static/null"}
#!/bin/sh
echo "<?"
echo "" >> code.php
tee -a code.php
...
```{.unwrap pipe="tee eps2png | root/static/null"}
#!/bin/sh
...
The Fibonacci sequence is a well-known series of numbers which follow a pattern: it starts `<? 1, 1`{.php} and each subsequent number is the sum of the two preceding it. We can calculate the `<? $n`{.php}th element of the sequence like this in PHP (using handy functions from [PHP Prelude](/git/php-prelude)):
...
```{.unwrap pipe="./runphp | tee mkfib1.data | root/static/null"}
...
```{.unwrap pipe="gnuplot | root/static/null"}
reset
...
```{.unwrap pipe="./runphp | tee mkfib12.data | root/static/null"}
$data = map(function($x) {
...
```{.unwrap pipe="./runphp | tee mkfibs34.data | root/static/null"}
