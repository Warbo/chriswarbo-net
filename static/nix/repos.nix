...
{ attrsToDirs', isPath, jq, lib, redirect, render, repoSource, runCommand, wget
, xidel }:
...
with rec {
  ...
  repoDirs = if isPath repoSource || isAbsPath repoSource then
    attrNames (filterAttrs dirIsRepo (readDir repoSource))
  else
    import (runCommand "repoUrls.nix" {
      inherit repoSource;
