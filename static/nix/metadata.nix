...
with rec {
  inherit (builtins)
    attrNames currentTime filter getAttr map readDir toJSON toString;
  inherit (lib)
    concatMapStringsSep concatStringsSep escapeShellArg fold hasPrefix
    hasSuffix;

  markdownPaths = with rec {
    dirEntries = dir:
      ...
      (filter (n: !(hasPrefix "." n)) (attrNames entries));

    go = { path, type }:
      rest:
      if type == "directory" then
        fold go rest (dirEntries path)
      else
        rest ++ (if hasSuffix ".md" (toString path) then
          [ (toString path) ]
        else
          [ ]);
  };
