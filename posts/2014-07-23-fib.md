...
```{pipe="tee composer.json > /dev/null"}
{"name"    : "fib",
...
```{pipe="tee hide > /dev/null"}
#!/bin/sh
echo "" >> code.php
tee -a code.php > /dev/null
```
...
```{pipe="tee init.php > /dev/null"}
<?php
include_once('code.php');

```

```{pipe="tee runphp > /dev/null"}
#!/bin/sh
...
```{pipe="tee eps2png > /dev/null"}
#!/bin/sh
convert  -density 128 "$1.eps" "$1big.png"
pngcrush -brute "$1big.png" "$1.png"
...
```{pipe="./runphp | tee mkfib1.data > /dev/null"}
...
```{pipe="./runphp | tee mkfib12.data > /dev/null"}
$data = map(function($x) {
...
```{pipe="./runphp | tee mkfibs34.data > /dev/null"}
