...
<!-- PHP blocks begin with "<?" for syntax highlighting; this trims it off -->
...
<!-- GNUPlot's default rendering is ugly, so override it -->
...
The Fibonacci sequence is a well-known series of numbers which follow a pattern: it starts `1, 1` and each subsequent number is the sum of the two preceding it. We can calculate the `<? $n`{.php}th element of the sequence like this in PHP (using handy functions from [PHP Prelude](https://gitorious.org/php-prelude)):
...
set logscale y2
plot "mkfib1.data" using 1:2 with line title "Recursive calls" linetype 1 lw 2, \
     "mkfib1.data" using 1:3 with line title "Time" linetype 2 lw 2 axes x1y2
```
...
```{.hidden pipe="sh"}
exit 0
echo 'echo fib_bench("9")'       | ./runphp
echo 'echo fib_bench_base(null)' | ./runphp
echo 'echo fib_mem("9")'         | ./runphp
echo 'echo fib_mem_base(null)'   | ./runphp
```
